...
class CreateDatabaseFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.title_label.text_key = "main_sidebar_create"
        ...
        self.links_file_entry = ctk.CTkEntry(self.path_frame, placeholder_text="Links Txt File or Folder")
        ...
        self.saving_folder_entry = ctk.CTkEntry(self.path_frame, placeholder_text="Saving Folder")
        self.saving_folder_entry.grid(row=1, column=0, columnspan=2, padx=(10, 0), pady=10, sticky="ew")

        self.saving_folder_btn = ctk.CTkButton(self.path_frame, text="Browse", width=80, command=self.browse_saving_folder)
        self.saving_folder_btn.grid(row=1, column=2, padx=(10, 10), pady=10)
        self.saving_folder_btn.text_key = "btn_browse"
    ...
    def browse_saving_folder(self):
        foldername = filedialog.askdirectory()
        if foldername:
            self.saving_folder_entry.delete(0, 'end')
            self.saving_folder_entry.insert(0, foldername)

    def start_process(self):
        links_f = self.links_file_entry.get()
        save_d = self.saving_folder_entry.get()
        if not links_f or not save_d: return
        ...
        # 5. Thread Execution
        self.worker = DatabaseBuilderThread(
            links_file=links_f,
            saving_folder=save_d,
            languages=langs,
            translate_missing=self.trans_var.get(),
            is_all_set=self.set_choice_var.get() == 1,
            delay_ms=delay_ms,
    ...
    def save_progress(self):
        ...
        state = {
            "ui_config": {
                ...
                "links_file": self.links_file_entry.get(),
                "saving_folder": self.saving_folder_entry.get(),
                "export_formats": {
                    'all': self.export_all_var.get(),
                    'csv': self.export_csv_var.get(),
                    'json': self.export_json_var.get(),
            ...
            "worker_state": {
                "remaining_urls": self.worker.urls if self.worker else [],
        ...
        self.after(0, lambda: self.status_label.configure(text=f"Progress Saved: {os.path.basename(filename)}", text_color="green"))
    ...
    def load_progress(self):
        ...
        self.links_file_entry.delete(0, 'end')
        self.links_file_entry.insert(0, ui.get("links_file", ""))
        self.saving_folder_entry.delete(0, 'end')
        self.saving_folder_entry.insert(0, ui.get("saving_folder", ""))
        ...
        if remaining:
            save_dir = self.saving_folder_entry.get() or "."
        ...
        self.after(0, lambda: self.status_label.configure(text="Progress Loaded. Ready to Continue.", text_color="blue"))
